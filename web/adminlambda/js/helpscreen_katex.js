...
function renderKatexHelpPreview() {
    const destval = getFormulaText($('#helpTextSrc').val() || '');
    $('#helpPreview').html(destval);
}

function renderKatexHelpSamples() {
    $('.katex-help-sample-result').each(function () {
        const source = $(this).text() || '';
        $(this).html(getFormulaText(source));
    });
}
