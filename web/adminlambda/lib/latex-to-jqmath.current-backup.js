...
function latex_to_js(input) {
    var st1 = input;
    st1 = normalizeChemistryBondChains(st1);
...
function normalizeChemistryBondChains(input) {
    input = input.replace(
        /\\underset\s*\{\\text\s*\{([^{}]+)\}\}\s*\{\s*H-\s*\\overset\s*\{\s*\\overset\s*\{\s*O\s*\}\s*\{\s*\|\|\s*\}\s*\}\s*\{\s*C\s*\}\s*-\s*H,?\s*\}/g,
        (_, label) => {
            const cleaned = label.trim().replace(/\b([0-9.]+)\s+moles\b/i, "$1 \\moles");
            return `H-C↙{${cleaned}}↖{\\|\\|}↖{O}-H,`;
        }
    );
    input = input.replace(
        /H-\s*\\overset\s*\{\s*\\overset\s*\{\s*O\s*\}\s*\{\s*\|\|\s*\}\s*\}\s*\{\s*C\s*\}\s*-\s*\\underset\s*\{\s*\\underset\s*\{\s*O\s*\}\s*\{\s*\|\|\s*\}\s*\}\s*\{\s*C\s*\}\s*-\s*\\overset\s*\{\s*\\overset\s*\{\s*O\s*\}\s*\{\s*\|\|\s*\}\s*\}\s*\{\s*C\s*\}\s*-\s*H,?/g,
        () => `H-C↖{\\|\\|}↖{O}-C↙{\\|\\|}↙{O}-C↖{\\|\\|}↖{O}-H,`
    );
    return input;
}
...
function performLatexOperations(st1) {
    ...
    st1 = chemistryChainUnderset(st1);
...
function chemistryChainUnderset(input) {
    const pattern = /\\underset\s*\{((?:[^{}]|\{[^{}]*\})*)\}\s*\{H-\s*([A-Za-z]+)((?:↖\{[^{}]+\})*)\s*-\s*H,?\s*\}/g;
    while (pattern.test(input)) {
        input = input.replace(pattern, (match, one, two, three) => `H-${two}↙{${one.trim()}}${three.replace(/\|\|/g, "\\|\\|")}-H`);
    }
    input = input.replace(/-\s+([A-Z])/g, "-$1");
    return input;
}
